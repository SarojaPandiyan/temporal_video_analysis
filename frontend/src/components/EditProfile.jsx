...
export default function EditProfile({ onCancel, isDark }) {
  ...
  return (
    <div
      className={`fixed inset-0 flex items-center justify-center ${themeBg}`}
    >
      {/* Card */}
      <div
        className={`w-[90%] max-w-md rounded-xl ${isDark ? "text-white bg-[#1f1f1f]" : "text-black bg-gray-200"} p-6}`}
      >
        <div className="my-5 mx-3">
          ...
          <div className="relative mx-auto mb-6 flex h-28 w-28 items-center justify-center rounded-full bg-[#8a9a9a] text-3xl font-medium">
            SP
            <div
              className={`absolute bottom-1 right-1 rounded-full ${isDark ? "bg-[#2a2a2a]" : "bg-white"} p-2 text-xs`}
            >
              <FaPhotoVideo />
            </div>
          </div>
          ...
          <div className="mb-4">
            <label
              className={`mb-2 block text-sm ${isDark ? "text-gray-400" : "text-black"}`}
            >
              Display Name
            </label>
            <input
